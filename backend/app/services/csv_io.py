...
def _parse_full_story_metadata(
    ...
    *,
    use_metadata_tropes_on_mismatch: bool = False,
) -> tuple[StoryCompleteness, int | None, list[str], list[tuple[StoryTropeOrigin, AssignmentStatus]]]:
    ...
    for assignment in assignments:
        ...
        trope_text = clean_text(assignment.get("text", ""))
        if not normalize_text(trope_text):
            raise CSVImportValidationError(
                f"Data row {row.source_row_number} has trope metadata in a different order from Motifs (Eng)."
            )
        try:
        ...
        metadata_trope_texts.append(trope_text)
        parsed_assignments.append((origin, status))

    matches_legacy_cell = len(metadata_trope_texts) == len(trope_texts) and all(
        normalize_text(metadata_trope_text) == normalize_text(legacy_trope_text)
        for metadata_trope_text, legacy_trope_text in zip(metadata_trope_texts, trope_texts, strict=True)
    )
    if not matches_legacy_cell:
        if not use_metadata_tropes_on_mismatch:
            message = (
                "has trope metadata that does not match Motifs (Eng)."
                if len(metadata_trope_texts) != len(trope_texts)
                else "has trope metadata in a different order from Motifs (Eng)."
            )
            raise CSVImportValidationError(f"Data row {row.source_row_number} {message}")
        trope_texts = metadata_trope_texts

    return completeness, original_source_row_number, trope_texts, parsed_assignments
...
def import_csv_bytes(
    ...
    *,
    ...
    use_full_export_trope_metadata_on_mismatch: bool = False,
) -> Dataset:
    """Import a legacy CSV or full Marawa export into a staged dataset.

    Normal full-export imports reject mismatches between the legacy trope cell
    and its metadata. Time Machine recovery can opt in to the metadata because
    it reads a verified archive generated by Marawa itself; this preserves a
    trope containing legacy delimiters that cannot be serialized back into the
    legacy cell unambiguously.
    """
    ...
    for row in rows:
        ...
        if is_full_export:
            completeness, source_row_number, tropes, trope_assignment_metadata = _parse_full_story_metadata(
                row,
                tropes,
                use_metadata_tropes_on_mismatch=use_full_export_trope_metadata_on_mismatch,
            )
        else:
