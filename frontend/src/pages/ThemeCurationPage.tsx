          ...
          <div className="stack">
            {pairs?.items.length ? pairs.items.map((pair) => {
              ...
              return <article className="card" key={id}>
                ...
                <div className="field-grid"><div className="stack"><strong>Source</strong><TermCard className="subdued" term={source} /></div><div className="stack"><strong>Target</strong><TermCard className="subdued" term={target} actions={<ConfirmationStatusSwitch ariaLabel={`Confirmation status for ${target.text}`} disabled={mutationDisabled || !target.version} onChange={(status) => void updateTargetStatus(target, status)} value={target.confirmation_status ?? "unconfirmed"} />} /></div></div>
